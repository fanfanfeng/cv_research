...
from classification.Inception.slim import losses
from classification.Inception.slim import scopes
from classification.Inception.slim import variables
...
def batch_norm(inputs,
    ...
    with tf.variable_scope(scope,'BatchNorm',[inputs],reuse=reuse):
        ...
        if sclae:
            gama = variables.variable("gamma",params_shape,initializer=tf.ones_initializer(),trainable=trainable,restore=restore)
        ...
        # Mormalize the activations.
        outputs = tf.nn.batch_normalization(inputs,mean,variance,beta,gama,epsilon)
...
def conv2d(inputs,
    ...
    with tf.variable_scope(scope,'Conv',[inputs],reuse=reuse):
        ...
        weights = variables.variable('weights',
                                     ...
                                     trainable=trainable,
                                     restore=restore)
        conv = tf.nn.conv2d(inputs,weights,[1,stride_h,stride_w,1],padding=padding)
        if batch_norm_params is not None:
            with scopes.arg_scope([batch_norm],is_training=is_training,trainable=trainable,restore=restore):
